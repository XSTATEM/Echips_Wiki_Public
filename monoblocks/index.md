...
import { data as monoblockLines } from './lines.data.mjs'
    ...
    >
      ...
      <div class="card-inner">
        <div class="laptop-image-container">
          <img :src="line.image" :alt="line.title" class="laptop-img" onerror="this.style.display='none'">
        </div>
        <h2 class="card-h">{{ line.title }}</h2>
...
<style scoped>
/* (Сюда вставь весь тот же блок <style scoped>, который ты мне скинула, ничего в нем не меняй) */
...
.echips-wrapper h1, .echips-wrapper h2, .echips-wrapper h3 { border: none !important; margin: 0; padding: 0; }
.echips-wrapper { --e-yellow: #FFB800; --e-orange: #FF4500; --e-gray: #8E8E93; --e-text: var(--vp-c-text-1); --e-desc: var(--vp-c-text-2); max-width: 1100px; margin: 0 auto; padding: 60px 20px 80px; position: relative; text-align: center; font-family: 'Montserrat', sans-serif !important; }
...
.laptop-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 32px; animation: fadeSlideUp 0.8s ease forwards; }
.laptop-card { position: relative; padding: 40px 24px; border-radius: 28px; display: flex; flex-direction: column; align-items: center; justify-content: space-between; background: var(--vp-c-bg-soft); border: 1px solid rgba(128, 130, 133, 0.25) !important; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.08) !important; transition: all 0.4s cubic-bezier(0.2, 0.8, 0.2, 1); overflow: hidden; }
html.dark .laptop-card { border: 1px solid rgba(255, 255, 255, 0.12) !important; box-shadow: 0 16px 40px rgba(0, 0, 0, 0.3) !important; }
...
.laptop-img { max-width: 90%; max-height: 100%; object-fit: contain; filter: drop-shadow(0 15px 25px rgba(0,0,0,0.25)); transition: transform 0.5s cubic-bezier(0.2, 0.8, 0.2, 1); }
.card-h { font-size: 24px; font-weight: 800; color: var(--e-text); margin: 0 0 20px 0 !important; text-align: center; }
.btn-yellow { font-size: 14px; font-weight: 600; color: var(--e-text); background: var(--vp-c-default-soft); padding: 10px 24px; border-radius: 20px; transition: all 0.3s ease; display: inline-block; pointer-events: auto; }
...
.mouse-glow { position: absolute; top: 0; left: 0; right: 0; bottom: 0; opacity: 0; transition: opacity 0.5s ease; pointer-events: none; z-index: 0; background: radial-gradient(circle 200px at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(255, 184, 0, 0.8), transparent 100%); }
...
@media (max-width: 800px) { .hero-title { font-size: 38px; } .laptop-grid { grid-template-columns: 1fr; } }
